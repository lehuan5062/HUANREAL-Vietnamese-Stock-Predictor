"""Portfolio simulation: buy the daily best rebound pick, held tickers EXCLUDED.

Currently-held tickers are removed from the candidate pool each day, so the buy
target is the highest-ranked pick you DON'T already own. You still buy every
day (unless every candidate that day is already held) — you just never add to a
name already in the portfolio.

Rules:
  * BUY DAILY   — every trading day, target that day's best-ranked pick that is
                  **NOT already held** (walk down the ranked list, skip owned).
...
Fees explicit at each leg. Unsold at end = loss.
    ...
    python -m scripts.rebound_sim_exclude_held
"""
...
def simulate(data=None):
    """Simulate with hardcoded limit_next_day execution + exclude-held candidate filter.
    ...
    n_signals = 0
    n_missed_fills = 0
    n_days_all_held = 0  # days where every candidate was already owned
    positions = []
    cashflows = []
    trades = []
    equity = []
    ...
    for date in days:
        ...
        # 2) BUY — best-ranked pick NOT already held (EXCLUDE held from candidates).
        held_syms = {p["sym"] for p in positions}
        cands = [c for c in per_day.get(date, []) if c["symbol"] not in held_syms]

        all_cands = per_day.get(date, [])
        if all_cands and not cands:
            n_days_all_held += 1  # had candidates, but all already owned

        if cands:
            c = cands[0]  # top-ranked UNHELD pick
    ...
    return {
        ...
        "days_all_candidates_held": n_days_all_held,
        "signals": n_signals,
