...
hold at least the model's predicted days before selling.

Same rules as rebound_sim_include_held.py, except the earliest sellable day
is max(T+2, pred_days) from entry instead of a flat T+2. pred_days is the
Kaplan-Meier estimator's days-to-recovery figure for that pick — this is a
minimum-hold GATE, not a forced exit day: if the position is still
unprofitable when pred_days elapses, it keeps waiting for the first
profitable close after that, exactly like the T+2 gate does today.
...
Rules:
  ...
  * EXIT        — hold at least max(T+2, pred_days) from entry, then sell on
...
Fees explicit at each leg. Unsold at end = loss.
    ...
    python -m scripts.rebound_sim_hold_pred_days
"""
...
def simulate(data=None):
    ...
    for date in days:
        # 1) SELLS — from max(T+2, pred_days) after entry, exit on first profitable close.
        still = []
        for pos in positions:
            j, c_t = bar_on(date, pos["sym"])
            min_hold = max(SETTLE_DAYS, round(pos["pred_days"])) if pos.get("pred_days") else SETTLE_DAYS
            if j is not None and j >= pos["entry_j"] + min_hold \
